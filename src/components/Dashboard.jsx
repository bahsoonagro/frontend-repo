...
const Dashboard = ({ apiUrl }) => {
  ...
  const rawMaterialData = rawMaterials
    ...
    .map(r => ({ name: r.rawMaterialType || "Unnamed", quantity: Number(r.bagsAfterStd || 0) }));
  ...
  const handleLineClick = (data) => {
    if (data && data.activeLabel) {
      const monthProducts = filteredDispatches.filter(d => {
        ...
        return month === data.activeLabel;
      }).map(d => d.item || d.productName).filter(Boolean);
      if (monthProducts.length === 1) setSelectedProduct(monthProducts[0]);
      else setSelectedProduct("all");
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          {label:"Finished Products", icon:FaBox, value:totalFinishedProducts, type:"finished", color:"green-500"},
          {label:"Dispatches", icon:FaTruck, value:totalDispatches, type:"dispatch", color:"purple-500"},
        ...
        <div className="bg-white shadow-md rounded-lg p-4 hover:shadow-xl transition transform hover:scale-105">
          ...
          {monthlyDispatch.length === 0 ? <p className="text-gray-500">No dispatch data available</p> :
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyDispatch} onClick={handleLineClick}>
