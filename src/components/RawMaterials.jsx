...
const RAW_MATERIALS_TABS = ["Sorghum", "Pigeon Peas", "Sesame Seeds", "Rice", "Sugar"];// src/components/RawMaterials.jsx
import React, { useState, useEffect, useRef } from "react";
import { Box, Button, Grid, TextField, Paper, Typography, IconButton } from "@mui/material";
import { Delete, Edit, Print, Inventory } from "@mui/icons-material";
import { motion, AnimatePresence } from "framer-motion";
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box ref={printRef}>
        <Box display="flex" justifyContent="flex-end" gap={2} mb={1}>
          ...
          <Button variant="outlined" onClick={() => exportExcel(materials, currentTab)}>
            Export Excel
          </Button>
          <Button variant="outlined" onClick={() => exportPDF(materials, currentTab)}>
            Export PDF
          </Button>
        </Box>
...
// Helper functions for Excel & PDF export
function exportExcel(materials, currentTab) {
...
function exportPDF(materials, currentTab) {
  const doc = new jsPDF();
  const filtered = materials.filter((m) => m.rawMaterialType === RAW_MATERIALS_TABS[currentTab]);
